...
Supports two table layouts:
  1. Multi-column  — separate date / description / debit / credit / amount columns.
  2. Single-column — HDFC-style where every row is one merged text cell:
       DD/MM/YYYY| HH:MM  MERCHANT [+ N C]  C AMOUNT.XX [l]
     (The PDF renderer replaces ₹ with "C" and appends a trailing "l".)
...
Public API used by the router and the playground notebook:
  ...
  _parse_table(table)             -> (rows, skipped_count)
...
_HDR = {
    ...
    "debit": re.compile(r"\b(debit|dr\.?|withdrawal)\b", re.IGNORECASE),
    "credit": re.compile(r"\b(credit|cr\.?|deposit)\b", re.IGNORECASE),
...
_TXN_HEADER_RE = re.compile(
    r"\b(date|transaction|description|narration|debit|credit|amount"
    r"|withdrawal|deposit)\b",
    re.IGNORECASE,
)


def _is_transaction_table(table: List[List[str]]) -> bool:
    """Return True only if the table's first rows look like transaction data."""
    if not table:
        return False
    header_text = " ".join(c for c in table[0] if c)
    return bool(_TXN_HEADER_RE.search(header_text))


def _parse_table(table: List[List[str]]) -> Tuple[List[ParsedRow], int]:
    """
    ...
    for row in table[start_row:]:
        ...
        if is_single_col:
            ...
            parsed = _parse_multi_column_row(row, col_map)
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for raw_table in tables:
                ...
                page_rows, page_skipped = _parse_table(table)
